...
class DuckDBFlightSqlServer::Impl {
    duckdb_database db_instance_;
    duckdb_connection db_conn_;

  public:
    explicit Impl(
      duckdb_database db_instance,
      duckdb_connection db_connection
...
arrow::Result<std::shared_ptr<DuckDBFlightSqlServer>> DuckDBFlightSqlServer::Create(
    ...
    if (duckdb_open_ext(path, &db, config, db_open_error) == DuckDBError) {
